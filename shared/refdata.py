...
COLLECTED_RESOURCES = {
    "history": RESOURCE_HISTORY,     # ~2.4 млн строк — пробег + номер двигателя
    # Основной реестр собираем РАДИ ОДНОГО ПОЛЯ — mivchan_acharon_dt (дата теста).
    # В ресурсе history даты теста нет вообще, только сам километраж, поэтому без
    # основного реестра невозможно сказать, НА КАКУЮ ДАТУ снят этот пробег —
    # получалась бы история вида "пробег такой-то, замечен нашим скриптом в среду",
    # что бесполезно. С ним получается честное "пробег X на тесте от 07.05.2026".
    "main": RESOURCE_MAIN,            # ~4.2 млн строк
    "ownership": RESOURCE_OWNERSHIP,  # ~5.3 млн строк — история владения
}

# Какие колонки запрашивать у API для каждого собираемого ресурса.
# Смысл только для основного реестра: там 24 колонки, а нужны две — без этого
# ограничения процесс на его загрузке разрастался до ~2 ГБ памяти, что рискованно
# для раннера GitHub Actions. Ресурсы, которых тут нет, тянутся целиком.
COLLECTED_RESOURCE_FIELDS = {
    "main": [
        "mispar_rechev",        # ключ
        "mivchan_acharon_dt",   # дата теста -> odometer_readings
        # ниже — поля, изменения которых пишем в журнал (см. TRACKED_MAIN_FIELDS)
        "tzeva_rechev", "misgeret", "baalut", "sug_delek_nm",
        "degem_manoa", "degem_cd", "degem_nm",
    ],
}

# Поля основного реестра, за изменениями которых следим (журнал field_changes).
# Ключ — имя колонки в реестре, значение — читаемая подпись.
# Тип КПП (automatic_ind) сюда не входит осознанно: в реестре по конкретной
# машине его нет вообще, он есть только в каталоге МОДЕЛЕЙ, т.е. является
# свойством модели, а не экземпляра. Ближайший честный сигнал — смена degem_cd:
# если машину переклассифицировали в другую модель, то и характеристики
# (включая КПП), которые к ней подтягиваются, становятся другими.
TRACKED_MAIN_FIELDS = {
...
# Поля, за изменениями которых следит сборщик, и их читаемые названия для
# журнала изменений (таблица field_changes в collector/db.py и changes в Turso).
# Ключи — ВНУТРЕННИЕ имена колонок current_state, а не имена полей data.gov.il.
TRACKED_FIELD_LABELS = {
    "km": "קילומטראז' במבחן אחרון",
    "engine_no": "מספר מנוע",
    "shinui_mivne_ind": "שינוי מבנה",
    "gapam_ind": "מותקן גפ\"ם",
    "shnui_zeva_ind": "שינוי צבע",
...
# Читаемые названия типов записи в журнале изменений (для интерфейса на иврите).
CHANGE_KIND_LABELS = {
