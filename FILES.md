...
├── .gitignore                              ← исключает db/, archive/, logs/, секреты сервера, .venv, dist/
...
│   ├── api_loader.py                       ← постраничная загрузка ресурсов через JSON API в staging
│   ├── csv_loader.py                       ← разбор CSV (сейчас не используется, см. api_loader.py)
│   ├── collector.py                        ← точка входа: python collector/collector.py
│   ├── db/                                 ← ПУСТАЯ папка на старте — carcheck.sqlite3 создастся сама
│   ├── archive/                            ← ПУСТАЯ папка на старте — снимки ответов API (.jsonl.gz)
...
└── export/
    ...
    ├── turso_upload.py                      ← заливает историю по машинам в Turso (отдельно от сайта!)
...
попадают ни в `dist/`, ни в git вообще — они уходят напрямую в Turso через
`turso_upload.py` и её HTTP API. В репозитории и на самом сайте лежит только код
